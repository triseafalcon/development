jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(SEAFOOD_COMPANY_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == SEAFOOD_COMPANY_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(SEAFOOD_COMPANY_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				seafood_company_message_success('', SEAFOOD_COMPANY_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(SEAFOOD_COMPANY_STORAGE['strings']['editor_caption_close']);
			} else {
				seafood_company_message_warning(rez.error, SEAFOOD_COMPANY_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		seafood_company_message_confirm(SEAFOOD_COMPANY_STORAGE['strings']['editor_delete_post'], SEAFOOD_COMPANY_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(SEAFOOD_COMPANY_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					seafood_company_message_success('', SEAFOOD_COMPANY_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = SEAFOOD_COMPANY_STORAGE['site_url'];
						}, 1000);
				} else {
					seafood_company_message_warning(rez.error, SEAFOOD_COMPANY_STORAGE['strings']['editor_delete_error']);
				}
			});
