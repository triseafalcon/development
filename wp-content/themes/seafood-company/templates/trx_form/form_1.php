...
if ( !function_exists( 'seafood_company_template_form_1_theme_setup' ) ) {
	add_action( 'seafood_company_action_before_init_theme', 'seafood_company_template_form_1_theme_setup', 1 );
	function seafood_company_template_form_1_theme_setup() {
		seafood_company_add_template(array(
			'layout' => 'form_1',
			'mode'   => 'forms',
			'title'  => esc_html__('Contact Form 1', 'seafood-company')
			));
	}
}

// Template output
if ( !function_exists( 'seafood_company_template_form_1_output' ) ) {
	function seafood_company_template_form_1_output($post_options, $post_data) {
		...
		<form <?php echo !empty($post_options['id']) ? ' id="'.esc_attr($post_options['id']).'_form"' : ''; ?>
			...
			<div class="sc_form_info columns_wrap">
				<div class="sc_form_item sc_form_field label_over column-1_2"><input id="sc_form_username" type="text" name="username"<?php if ($form_style=='default') echo ' placeholder="'.esc_attr__('Enter your name', 'seafood-company').'"'; ?> aria-required="true"><?php
				...
				?></div><div class="sc_form_item sc_form_field label_over column-1_2"><input id="sc_form_email" type="text" name="email"<?php if ($form_style=='default') echo ' placeholder="'.esc_attr__('Enter your email', 'seafood-company').'"'; ?> aria-required="true"><?php
					if ($form_style!='default') { 
...
<?php } ?>
			...
			<div class="sc_form_item sc_form_message"><textarea id="sc_form_message" name="message"<?php if ($form_style=='default') echo ' placeholder="'.esc_attr__('Enter your message', 'seafood-company').'"'; ?> aria-required="true"></textarea><?php
				if ($form_style!='default') { 
