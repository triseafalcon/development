...
if ( !function_exists( 'seafood_company_template_players_1_theme_setup' ) ) {
	add_action( 'seafood_company_action_before_init_theme', 'seafood_company_template_players_1_theme_setup', 1 );
	function seafood_company_template_players_1_theme_setup() { 
		seafood_company_add_template(array(
			'layout' => 'players-1',
			'template' => 'players-1',
			'mode'   => 'players',
			'title'  => esc_html__('Players /Style 1/', 'seafood-company'),
...
// Template output
if ( !function_exists( 'seafood_company_template_players_1_output' ) ) {
	function seafood_company_template_players_1_output($post_options, $post_data) { 
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_player_avatar"><?php seafood_company_show_layout($post_options['photo']); ?>
					<div class="sc_player_hover">
						<?php
						if (!empty($post_options['socials']) && $post_options['socials'] != 'inherit'){?>
							<div class="sc_player_socials"><?php seafood_company_show_layout($post_options['socials']); ?></div>
						<?php } ?>
					</div>
				</div>
				<div class="sc_player_info">
					...
					if(!empty($post_options['club']) && $post_options['club'] != 'inherit'){?>
						<div class="sc_player_club">
							<?php seafood_company_show_layout($post_options['club']);?>
						</div>
					<?php } ?>
				</div>
			</div>
