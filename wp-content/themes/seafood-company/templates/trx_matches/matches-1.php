...
if ( !function_exists( 'seafood_company_template_matches_1_theme_setup' ) ) {
	add_action( 'seafood_company_action_before_init_theme', 'seafood_company_template_matches_1_theme_setup', 1 );
	function seafood_company_template_matches_1_theme_setup() {
		seafood_company_add_template(array(
			'layout' => 'matches-1',
			'template' => 'matches-1',
			'mode'   => 'matches',
			'title'  => esc_html__('Matches /Style 1/', 'seafood-company'),
...
// Template output
if ( !function_exists( 'seafood_company_template_matches_1_output' ) ) {
	function seafood_company_template_matches_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<li<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				if (!$post_data['post_protected']) {
					...
					$match_preview = !empty($post_meta['match_link']) && !seafood_company_param_is_inherit($post_meta['match_link']) ? $post_meta['match_link'] : '';	
