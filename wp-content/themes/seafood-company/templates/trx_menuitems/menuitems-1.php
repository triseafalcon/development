...
if ( !function_exists( 'seafood_company_template_menuitems_1_theme_setup' ) ) {
	add_action( 'seafood_company_action_before_init_theme', 'seafood_company_template_menuitems_1_theme_setup', 1 );
	function seafood_company_template_menuitems_1_theme_setup() {
		seafood_company_add_template(array(
			'layout' => 'menuitems-1',
			'template' => 'menuitems-1',
			'mode'   => 'menuitems',
			'title'  => esc_html__('MenuItems /Style 1/', 'seafood-company'),
			'thumb_title'  => esc_html__('Avatar (small)', 'seafood-company'),
			'w'		 => 75,
			'h'		 => 75
		));
	}
...
// Template output
if ( !function_exists( 'seafood_company_template_menuitems_1_output' ) ) {
	function seafood_company_template_menuitems_1_output($post_options, $post_data) {
			...
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> 
				...
				if ( ($post_options['menuitem_price'] != 'inherit') and ( strlen($post_options['menuitem_price']) != 0 ) ) {
					?><div class="sc_menuitem_price"><?php
						seafood_company_show_layout($post_options['menuitem_price']);
					?></div><?php
				}
				...
				if ($show_title) {
					...
					if ($post_options['popup'] == 'yes')
						...
						echo '<a href="'.esc_url($post_data['post_link']).'">';
					seafood_company_show_layout($post_data['post_title']);
					?></a></h5><?php
				}
				?>
				
				<div class="sc_menuitem_description"><?php seafood_company_show_layout(seafood_company_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : seafood_company_get_custom_option('post_excerpt_maxlength_masonry'))) ?></div>
