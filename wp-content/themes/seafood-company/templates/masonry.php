...
if ( !function_exists( 'seafood_company_template_masonry_theme_setup' ) ) {
	add_action( 'seafood_company_action_before_init_theme', 'seafood_company_template_masonry_theme_setup', 1 );
	function seafood_company_template_masonry_theme_setup() {
		seafood_company_add_template(array(
			'layout' => 'masonry_2',
			'template' => 'masonry',
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile (different height) /2 columns/', 'seafood-company'),
		...
		seafood_company_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile /3 columns/', 'seafood-company'),
			'thumb_title'  => esc_html__('Medium image', 'seafood-company'),
			'w'		 => 370,
			'h_crop' => 216,
			'h'      => null
		...
		seafood_company_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Masonry tile /4 columns/', 'seafood-company'),
			'thumb_title'  => esc_html__('Medium image', 'seafood-company'),
			'w'		 => 370,
			'h_crop' => 209,
			'h'      => null
		...
		seafood_company_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile (equal height) /2 columns/', 'seafood-company'),
		...
		seafood_company_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile /3 columns/', 'seafood-company'),
			'thumb_title'  => esc_html__('Medium image (crop) C3', 'seafood-company'),
			'w'		 => 370,
			'h'		 => 216
		));
		seafood_company_add_template(array(
			...
			'mode'   => 'blog',
			'need_isotope' => true,
			'title'  => esc_html__('Classic tile /4 columns/', 'seafood-company'),
			'thumb_title'  => esc_html__('Medium image (crop)', 'seafood-company'),
			'w'		 => 370,
			'h'		 => 209
		));
		// Add template specific scripts
		add_action('seafood_company_action_blog_scripts', 'seafood_company_template_masonry_add_scripts');
	}
}

// Add template specific scripts
if (!function_exists('seafood_company_template_masonry_add_scripts')) {
	
	function seafood_company_template_masonry_add_scripts($style) {
		if (in_array(seafood_company_substr($style, 0, 8), array('classic_', 'masonry_'))) {
...
// Template output
if ( !function_exists( 'seafood_company_template_masonry_output' ) ) {
	function seafood_company_template_masonry_output($post_options, $post_data) {
					...
					<div class="post_descr">
						...
						if ($post_data['post_protected']) {
							...
							if (!seafood_company_param_is_off($post_options['readmore']) && !in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status'))) {
								seafood_company_show_layout(seafood_company_sc_button(array('link'=>$post_data['post_link'],'class'=>'read_more_button'), $post_options['readmore']));
							}
						}
						?>
					</div>
